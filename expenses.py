"""Работа с расходами: добавление, удаление и статистики"""
import datetime
import re
...
def get_today_statistics() -> str:
    """Возвращает стракой статистику расходов за сегодня"""
    cursor = db.get_cursor()
    cursor.execute("select sum(amount)"
                   "from expense where date(created)=date('now', 'localtime')")
    result = cursor.fetchone()
    if not result[0]:
        return "Сегодня еще нет расходов"
...
def delete_expense(row_id: int) -> None:
    """Удаляет сообщение по его идетнификатору"""
...
def _get_now_formated() -> str:
    """Возвращает сегодняшнюю дату строкой"""
    return _get_now_datetime().strftime("%Y-%m-%d %H:%M:%S")

def _get_now_datetime() -> datetime.datetime:
    """Возвращает сегодняшний datetime с учетом временной зоны Москвы"""
    tz = pytz.all_timezones("Europe/Moscow")
